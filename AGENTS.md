...
- When Codex runs `npm run build` in this repo, request escalated permissions
  immediately instead of trying a sandboxed build first. The sandboxed build is
  known to fail before type checking with a Turbopack panic mentioning
  `globals.css`, `creating new process`, `binding to a port`, and
  `Operation not permitted`; this is a sandbox permission issue, not an app code
  issue.
