...
const initialRows = seats.map(() => ({ playerId: "", score: "" }));
...
function adminErrorMessage(err: unknown) {
  return err instanceof Error ? err.message : "Unable to add match.";
}
...
export function AdminAddMatch() {
  ...
  const [rows, setRows] = useState<MatchPlayerFormRow[]>(initialRows);
  const [gameType, setGameType] = useState<"east" | "south">("south");
  const [gameDate, setGameDate] = useState(todayDateValue);
  const [gameHour, setGameHour] = useState(currentHourValue);
